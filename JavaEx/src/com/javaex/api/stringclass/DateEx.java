...
//		dateClassEx();
	...
	private static void calendarEx() {
		...
		System.out.printf("현재: %d년 %d월 %d일 %n", 
				...
				now.get(Calendar.MONTH) + 1 ,		// 중요 : 1월을 0 이기에 + 1 처리
				now.get(Calendar.DATE));
		
		// 년월일 정보를 알고 있을 때
		custom.set(2021,8,24);
		System.out.printf("custom: %d년 %d월 %d일 %n",
				custom.get(Calendar.YEAR),
				custom.get(Calendar.MONTH) +1 ,		// 중요 : 1월을 0 이기에 + 1 처리
		...
		System.out.printf("future: %d년 %d월 %d일 %n",
				...
				future.get(Calendar.MONTH) +1 ,		// 중요 : 1월을 0 이기에 + 1 처리
